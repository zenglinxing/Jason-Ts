...
class Othorhombic_2():
    def __init__(self,t=1,a=(1,1,1),
        ...
        length_=tuple(sorted( (self.a,
                               self.b,
                               self.c,
                               np.sqrt(self.a**2+self.b**2)/2,) )) # all lengths
        length_angle=[ [self.a,(np.pi/2,0)],
                      [self.b,(np.pi/2,np.pi/2)],
                      [self.c,(0,0)],
                      [np.sqrt(self.a**2+self.b**2)/2,(np.pi/2,
                                                       np.pi/4)] ]
        length=() # lengths eliminating the closer
        l_=length_[0]
        length=(l_,)
...
class Othorhombic_3():
    def __init__(self,t=1,a=(1,1,1),
        ...
        length_=tuple(sorted( (self.a,
                               self.b,
                               self.c,
                               np.sqrt(self.a**2+self.b**2+self.c**2)/2,) ))
        length_angle=[ [self.a,(np.pi/2,0)],
                      [self.b,(np.pi/2,np.pi/2)],
                      [self.c,(0,0)],
                      [np.sqrt(self.a**2+self.b**2+self.c**2)/2,
                       (np.arctan(np.sqrt(self.a**2+self.b**2)/self.c),
                        np.arctan(self.b/self.a),)] ]
        length=() # lengths eliminating the closer
        l_=length_[0]
        length=(l_,)
        # dic={length:((theta,phi),(theta,phi),...)}
...
class Othorhombic_4():
    def __init__(self,t=1,a=(1,1,1),
        ...
        # order
        # all lengths
        length_=tuple(sorted( (np.sqrt(self.a**2+self.b**2)/2,
                               np.sqrt(self.a**2+self.c**2)/2,
                               np.sqrt(self.b**2+self.c**2)/2,) ))
        length_angle=[ [np.sqrt(self.a**2+self.b**2)/2,(np.pi/2,np.pi/4)],
                      [np.sqrt(self.a**2+self.b**2)/2,(np.pi/2,-np.pi/4)],
                      [np.sqrt(self.a**2+self.c**2)/2,(np.pi/4,0)],
                      [np.sqrt(self.a**2+self.c**2)/2,(np.pi/4,np.pi)],
                      [np.sqrt(self.b**2+self.c**2)/2,(np.pi/4,np.pi/2)],
                      [np.sqrt(self.b**2+self.c**2)/2,(np.pi/4,-np.pi/2)],]
        length=() # lengths eliminating the closer
        l_=length_[0]
        length=(l_,)
